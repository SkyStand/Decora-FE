...
const CartPage = () => {
    ...
    const subtotal = selectedItems.reduce((acc, item) => acc + item.qty * (item.variants[0].diskon || item.variants[0].price), 0);
    const serviceFee = 15000;
    const promoDiscount = selectedItems.reduce((acc, item) => acc + (item.variants[0].price - item.variants[0].diskon) * item.qty, 0);
                            ...
                            <div key={item.id} className="border border-salmon-3 rounded-lg p-4 mb-4 flex">
                                ...
                                <div className="flex flex-col items-end">
                                    {item.variants[0].diskon ? (
                                        <>
                                            <span className="text-lg font-semibold text-blue-tua">{`Rp ${item.variants[0].diskon.toLocaleString('id-ID')},00`}</span>
                                            <div>
                                                <span className="text-sm text-gray-500 line-through mr-2">{`Rp ${item.variants[0].price.toLocaleString('id-ID')},00`}</span>
                                                <span className="text-sm text-red-500">{item.variants[0].diskon}</span>
                                            </div>
                                        </>
                                    ) : (
                                        <span className="text-lg font-semibold text-gray-900">{`Rp ${item.variants[0].price.toLocaleString('id-ID')},00`}</span>
                                    )}
